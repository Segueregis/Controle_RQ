...
df = pd.read_excel('dados_pagamento.xlsx')  # Mudança aqui para ler o arquivo Excel
df_cru = df.copy()
...
# Remover pontos da coluna 'Valor Pago' antes de converter para int
df['Valor Pago'] = df['Valor Pago'].str.replace('.', '', regex=False).astype(int)
...
options_team = [{'label': 'Todas UTs', 'value': 0}]
for i in df['UT'].unique():
...
def team_filter(team):
    if team == 0:
        mask = df['UT'].isin(df['UT'].unique())
    else:
        mask = df['UT'].isin([team])
    return mask
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    dbc.Row([
                        dbc.Col([
                            html.Legend("Controle de Orçamento", style={'font-size': '19px', 'font-weight': 'bold'})
                            # Ajustar tamanho da fonte e peso
                        ], sm=12, md=8),

                        dbc.Col([
                            ThemeSwitchAIO(aio_id="theme", themes=[url_theme1, url_theme2]),
                            html.Legend("UT JOHNSON")
                        ])
...
def graph1(month, toggle):
    ...
    df_1 = df_1.groupby(['UT', 'Consultor'])['Valor Pago'].sum()
    df_1 = df_1.sort_values(ascending=False)
    df_1 = df_1.groupby('UT').head(1).reset_index()

    fig2 = go.Figure(go.Pie(labels=df_1['Consultor'] + ' - ' + df_1['UT'], values=df_1['Valor Pago'], hole=.6))
...
def graph5(month, toggle):
    ...
    df_5 = df_5.groupby(['Consultor', 'UT'])['Valor Pago'].sum()
    df_5.sort_values(ascending=False, inplace=True)
    df_5 = df_5.reset_index()
    fig5 = go.Figure()
    fig5.add_trace(go.Indicator(mode='number+delta',
                                title={
                                    "text": f"<span>{df_5['Consultor'].iloc[0]} - Top Consultant</span><br><span style='font-size:70%'>Em vendas - em relação a média</span><br>"},
                                value=df_5['Valor Pago'].iloc[0],
                                number={'prefix': "R$"},
                                delta={'relative': True, 'valueformat': '.1%', 'reference': df_5['Valor Pago'].mean()}
                                ))

    df_6 = df_6.groupby('UT')['Valor Pago'].sum()
    df_6.sort_values(ascending=False, inplace=True)
    df_6 = df_6.reset_index()
    fig6 = go.Figure()
    fig6.add_trace(go.Indicator(mode='number+delta',
                                title={
                                    "text": f"<span>{df_6['UT'].iloc[0]} - Top Team</span><br><span style='font-size:70%'>Em vendas - em relação a média</span><br>"},
...
def graph7(toggle):
    ...
    df_7 = df.groupby(['Mês', 'UT'])['Valor Pago'].sum().reset_index()
    df_7_group = df.groupby('Mês')['Valor Pago'].sum().reset_index()

    fig7 = px.line(df_7, y="Valor Pago", x="Mês", color="UT")
    fig7.add_trace(go.Scatter(y=df_7_group["Valor Pago"], x=df_7_group["Mês"], mode='lines+markers', fill='tonexty',
...
def graph8(month, toggle):
    ...
    df_8 = df_8.groupby('UT')['Valor Pago'].sum().reset_index()
...
def graph11(month, team, toggle):
    ...
    select = html.H1("Todas UTs") if team == 0 else html.H1(team)
